...
/// Callback triggered when a counter exceeds its threshold.
///
/// Parameters: counter name (for example, "GET" or "uploaded"), threshold limit, current value.
...
impl RequestCounters {
    ...
    ///
    /// Returns (previous value, threshold).
    fn increment(&self, method: &Method, ordering: Ordering) -> (u64, u64) {
        match *method {
            Method::DELETE => (
                self.delete.0.fetch_add(1, ordering),
                self.delete_threshold.0.load(Ordering::Acquire),
            ),
            Method::GET => (
                self.get.0.fetch_add(1, ordering),
                self.get_threshold.0.load(Ordering::Acquire),
            ),
            Method::HEAD => (
                self.head.0.fetch_add(1, ordering),
                self.head_threshold.0.load(Ordering::Acquire),
            ),
            Method::POST => (
                self.post.0.fetch_add(1, ordering),
                self.post_threshold.0.load(Ordering::Acquire),
            ),
            Method::PUT => (
                self.put.0.fetch_add(1, ordering),
                self.put_threshold.0.load(Ordering::Acquire),
            ),
            _ => (0, 0),
        }
    }
    ...
    fn load(&self, method: Method, ordering: Ordering) -> u64 {
        match method {
    ...
    fn reset(&self, ordering: Ordering) -> RequestCounterValues {
    ...
    ///
    /// Returns (previous value, threshold).
    fn add_upload_traffic(&self, count: u64, ordering: Ordering) -> (u64, u64) {
    ...
    fn get_upload_traffic(&self, ordering: Ordering) -> u64 {
    ...
    ///
    /// Returns (previous value, threshold).
    fn add_download_traffic(&self, count: u64, ordering: Ordering) -> (u64, u64) {
    ...
    fn get_download_traffic(&self, ordering: Ordering) -> u64 {
        self.download.0.load(ordering)
    }

    /// Update the request threshold values.
    fn update_thresholds(&self, thresholds: &RequestCounterThresholds) {
...
pub struct SharedRequestCounters {
    ...
    path: PathBuf,
    threshold_callback: Option<ThresholdExceededCallback>,
}

// The threshold_callback (Box<dyn Fn>) prevents auto-deriving RefUnwindSafe, but the callback is
// only invoked from regular (non-panicking) counter increment paths, so this is safe to assert.
impl std::panic::RefUnwindSafe for SharedRequestCounters {}
...
impl SharedRequestCounters {
    ...
    /// Fire the threshold callback if `current` just crossed `threshold`.
    fn check_threshold(&self, counter_id: &str, threshold: u64, prev: u64, current: u64) {
        if threshold > 0 && current > threshold && prev <= threshold {
            if let Some(cb) = &self.threshold_callback {
                cb(counter_id, threshold, current);
            }
        }
    }
    ...
    pub fn increment(&self, method: Method, ordering: Ordering) -> u64 {
        let (prev, threshold) = self
            .shared_memory
            .data()
            .counters
            .increment(&method, ordering);
        self.check_threshold(method.as_str(), threshold, prev, prev + 1);
        prev
    }
    ...
    pub fn add_upload_traffic(&self, count: u64, ordering: Ordering) -> u64 {
        ...
        self.check_threshold("uploaded", threshold, prev, prev + count);
        prev
    }
    ...
    pub fn add_download_traffic(&self, count: u64, ordering: Ordering) -> u64 {
        ...
        self.check_threshold("downloaded", threshold, prev, prev + count);
        prev
    }
    ...
    /// Set the callback invoked when a counter exceeds its threshold.
    ///
    /// The callback receives the counter name (for example, "GET" or "uploaded"), the threshold
    /// limit, and the current value.
    pub fn set_threshold_callback(&mut self, callback: ThresholdExceededCallback) {
        self.threshold_callback = Some(callback);
    }
